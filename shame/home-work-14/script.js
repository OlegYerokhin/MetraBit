  ...
  if(valid === 'true') {
    function toRegisterNewAccount() {
      for(let i = 0; i < response.length; i++) {
        if(response[i].value.toLowerCase() === newLogin.value.toLowerCase()) {
      ...
      if(newPassword.value && repeatedPassword.value && newPassword.value !== repeatedPassword.value) {
        createMessage('You entered different passwords. Please edit them and try again', 'red')
      }

      if(newLogin.value && newPassword.value && repeatedPassword.value && newPassword.value === repeatedPassword.value) {
